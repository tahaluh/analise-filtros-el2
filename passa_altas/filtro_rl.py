...
# Desenho do circuito RL Passa-Alta
d = schemdraw.Drawing()
d += elm.Label().label("Filtro Passa-Alta RL", fontsize=16, loc='top').at((2,4))
...
# Módulo linear
mag_linear = (w*L_value) / np.sqrt(R_value**2 + (w*L_value)**2)

# Fase em graus
phase = np.arctan(R_value / (w*L_value)) * (180/np.pi)
