...
# aumentar L (mantendo R)
# curva desliza para a direita (aumenta fc), inclinação idem.
# A impedância de entrada não muda, mas o indutor fica maior.
# Mais energia magnética armazenada ⇒ pode afetar resposta transitória.

# Valores do filtro RL
# Objetivo: Subwoofer passa-baixas com frequência de corte em 100 Hz
R_value = 4       # 4 Ω
L_value = 63.7e-3 # 63.7 mH
...
d = schemdraw.Drawing()
d += elm.Label().label("Filtro Passa-Baixa RL (1ª Variação)", fontsize=16, loc='top').at((2,4))
...
plt.title("Resposta em Frequência - Filtro Passa-Baixa RL (1ª Variação) (ω)")
plt.ylabel("|H(jω)|")
...
plt.title("Resposta em Frequência - Filtro Passa-Baixa RL (1ª Variação) (Hz)")
