...
# Valores do filtro RL (segunda variação)
R_value = 500    # 500 Ω
L_value = 0.1    # 0.1 H
...
d = schemdraw.Drawing()
d += elm.Label().label("Filtro Passa-Baixa RL (2ª Variação)", fontsize=16, loc='top').at((4,4))
...
d += elm.Resistor().down().label(f'R = {R_value} Ω', loc='bottom')
d += elm.Ground()
d += elm.Line().right().length(d.unit*2)
...
plt.title("Resposta em Frequência - Filtro Passa-Baixa RL (2ª Variação)")
plt.ylabel("|H(jω)|")
