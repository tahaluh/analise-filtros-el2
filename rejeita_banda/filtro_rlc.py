...
# Notch mais estreito (aumenta L e diminui C)

# Valores do filtro — Variação A (f0 ~ 60 Hz, notch mais estreito)
R_value = 1e3      # 1 kΩ
L_value = 1.0      # 1 H
C_value = 7.036e-6 # 7.036 µF  (LC ≈ 1/(2π·60)^2)
...
d = schemdraw.Drawing()
d += elm.Label().label("Filtro Rejeita-Banda RLC Série (1ª Variação)", fontsize=16, loc='top').at((2,7))
...
plt.title("Resposta em Frequência - Filtro Rejeita-Banda RLC Série (1ª Variação) (ω)")
plt.ylabel("|H(jω)|")
...
plt.title("Resposta em Frequência - Filtro Rejeita-Banda RLC Série (1ª Variação) (Hz)")
